...
export class LLMHelper {
  ...
  private deputyKey: string | null = null;
  ...
  public setDeputyKey(key: string | null): void {
    this.deputyKey = key || null;
    console.log(`[LLMHelper] Deputy key ${key ? 'set' : 'cleared'}`);
  }

  private hasDeputy(): boolean {
...
- Do NOT wrap the JSON in markdown fences.`;
  ...
  /**
   * Generate a suggestion based on conversation transcript - Deputy-style
...
[END LANGUAGE OVERRIDE]\n\n`;
  ...
  public async chatWithGemini(message: string, imagePaths?: string[], context?: string, skipSystemPrompt: boolean = false, alternateGroqMessage?: string): Promise<string> {
    try {
      ...
      if (this.currentModelId === 'deputy') {
        const { CredentialsManager } = require('./services/CredentialsManager');
        const deputyKey = CredentialsManager.getInstance().getDeputyApiKey();
        if (deputyKey) {
          try {
            return await this.generateWithDeputy(userContent, openaiSystemPrompt, imagePaths);
          } catch (err: any) {
            console.warn('[LLMHelper] Deputy API failed in chatWithGemini, falling back to Gemini:', err.message);
      ...
      if (isMultimodal) {
        // MULTIMODAL PROVIDER ORDER: [Deputy] -> OpenAI -> Gemini Flash -> Claude -> Gemini Pro -> Groq -> Custom/Ollama
        if (this.hasDeputy()) {
          providers.push({ name: 'Deputy API', execute: () => this.generateWithDeputy(userContent, openaiSystemPrompt, imagePaths) });
        }
        ...
        // TEXT-ONLY: [Deputy] -> Groq -> Gemini Flash -> Gemini Pro -> OpenAI -> Claude
        if (this.hasDeputy()) {
          providers.push({ name: 'Deputy API', execute: () => this.generateWithDeputy(userContent, openaiSystemPrompt) });
        }
        if (this.groqClient) {
  ...
  public async generateContentStructured(message: string): Promise<string> {
    ...
    // Priority 7: Deputy API — used when no other provider is available, or as final fallback
    const deputyKeyForStructured = this.deputyKey || (() => {
      try { return require('./services/CredentialsManager').CredentialsManager.getInstance().getDeputyApiKey() || null; } catch { return null; }
    })();
    if (deputyKeyForStructured) {
      providers.push({
        name: 'Deputy API',
        execute: () => this.generateWithDeputy(message)
      });
    }

    if (providers.length === 0) {
      throw new Error('No reasoning model available. Please configure an OpenAI, Claude, Gemini, Groq, or Deputy API key.');
    }
   ...
   */
  /**
   * Routes AI generation through the Deputy API backend (Gemini-powered).
   */
  private async generateWithDeputy(userMessage: string, systemPrompt?: string, imagePaths?: string[]): Promise<string> {
    // Prefer the in-memory field; fall back to CredentialsManager for the direct-routing path
    // where currentModelId === 'deputy' but setDeputyKey() wasn't called yet.
    let deputyKey = this.deputyKey;
    if (!deputyKey) {
      const { CredentialsManager } = require('./services/CredentialsManager');
      deputyKey = CredentialsManager.getInstance().getDeputyApiKey() || null;
    }
    if (!deputyKey) throw new Error('Deputy API key not set');

    const endpointUrl = 'https://api.deputy.software/v1/chat';
    ...
    if (deputyKey === '__trial__') {
      ...
      headers['x-deputy-key'] = deputyKey;
    }
    ...
    //
    // Compress before sending: retina screenshots are 2-5 MB PNG; the Deputy API body limit
    ...
    if (!response.ok) {
      ...
      throw new Error(`Deputy API error ${response.status}: ${errData.error || 'unknown'}`);
    }
  ...
  public async * streamChatWithGemini(message: string, imagePaths?: string[], context?: string, skipSystemPrompt: boolean = false): AsyncGenerator<string, void, unknown> {
    ...
    if (isMultimodal) {
      // MULTIMODAL PROVIDER ORDER: [Deputy] -> OpenAI -> Gemini Flash -> Claude -> Gemini Pro -> Groq Scout 4
      if (this.hasDeputy()) {
        providers.push({ name: 'Deputy API', execute: () => this.streamWithDeputy(userContent, openaiSystemPrompt, imagePaths) });
      }
      if (this.openaiClient) {
      ...
      // TEXT-ONLY PROVIDER ORDER: [Deputy] → Groq → OpenAI → Claude → Gemini Flash → Gemini Pro
      if (this.hasDeputy()) {
        providers.push({ name: 'Deputy API', execute: () => this.streamWithDeputy(userContent, openaiSystemPrompt) });
      }
      if (this.groqClient) {
    ...
    // ============================================================
    const currentFamilyLabel = this.currentModelId === 'deputy' ? 'Deputy'
    ...
    // Deputy is always first when configured, regardless of which model is selected.
    // The sort above may have displaced it — restore it to position 0.
    if (this.hasDeputy() && providers[0]?.name !== 'Deputy API') {
      const idx = providers.findIndex(p => p.name === 'Deputy API');
      if (idx > 0) {
  ...
  public async * streamChat(
    ...
    // GROQ FAST TEXT OVERRIDE (Text-Only)
    // Two paths: local Groq key → call Groq directly; Deputy API only → send fast_mode:true
    ...
    if (this.groqFastTextMode && !isMultimodal) {
      if (this.groqClient) {
        ...
        // Local Groq failed — fall through to Deputy if available
      }
      if (this.hasDeputy()) {
        // streamWithDeputy → generateWithDeputy → sends fast_mode:true → server Groq pool
        console.log(`[LLMHelper] ⚡️ Groq Fast Text Mode Active (Streaming). Routing to Deputy server Groq pool...`);
        try {
          yield* this.streamWithDeputy(userContent, finalSystemPrompt);
          return;
        } catch (e: any) {
          console.warn("[LLMHelper] Deputy fast-mode failed, falling back:", e.message);
        }
      }
    }
    ...
    if (this.currentModelId === 'deputy') {
      ...
      if (deputyKey) {
        try {
          const response = await this.generateWithDeputy(userContent, finalSystemPrompt, imagePaths);
          yield response;
          return;
        } catch (err: any) {
          console.warn('[LLMHelper] Deputy API failed in streamChat, trying Groq fallback:', err.message);
    ...
    // 5. Last-resort: Deputy API (if user has a key but no cloud provider configured)
    if (this.hasDeputy()) {
      try {
        yield* this.streamWithDeputy(userContent, finalSystemPrompt, imagePaths);
        return;
      } catch (e: any) {
        console.warn('[LLMHelper] Deputy last-resort fallback failed:', e.message);
      }
    }
  ...
  /**
   * Fake-stream for Deputy API (non-streaming endpoint).
   ...
   */
  private async * streamWithDeputy(userContent: string, systemPrompt?: string, imagePaths?: string[]): AsyncGenerator<string, void, unknown> {
    // ── REAL SSE STREAM (replaces the fake word-by-word simulation) ──────────
    // Previous implementation called generateWithDeputy() (blocking, waited for
    ...
    if (deputyKey === '__trial__') {
      ...
      streamHeaders['x-deputy-key'] = deputyKey;
    }
    ...
    const response = await fetch('https://api.deputy.software/v1/chat', {
      method:  'POST',
    ...
    if (!response.ok) {
      ...
      throw new Error(`Deputy API ${response.status}: ${(errData as any).error || 'unknown'}`);
    }
  ...
  public async generateMeetingSummary(systemPrompt: string, context: string, groqSystemPrompt?: string): Promise<string> {
    ...
    // ATTEMPT 1: Deputy API (if configured — first in chain)
    if (this.hasDeputy()) {
      try {
        console.log(`[LLMHelper] Attempting Deputy API for summary...`);
        const text = await this.withTimeout(
          this.generateWithDeputy(`Context:\n${context}`, systemPrompt),
          60000,
          'Deputy Summary'
        );
        if (text.trim().length > 0) {
          console.log(`[LLMHelper] ✅ Deputy API summary generated successfully.`);
          return this.processResponse(text);
        }
      } catch (e: any) {
        console.warn(`[LLMHelper] ⚠️ Deputy API summary failed: ${e.message}. Falling back...`);
      }
    }
