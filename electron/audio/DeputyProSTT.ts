 ...
 * DeputyProSTT
 *
 * Connects to the Deputy API WebSocket transcription endpoint.
 ...
 */
export class DeputyProSTT extends EventEmitter {
    ...
    private readonly BACKEND_URL = 'wss://api.deputy.software/v1/transcribe';
    ...
    public setSampleRate(rate: number): void {
        ...
        console.log(`[DeputyProSTT:${this.channel}] Sample rate configured to ${rate}Hz`);
    }
    ...
    public setRecognitionLanguage(key: string): void {
        ...
        if (key === 'auto') {
            ...
            console.log('[DeputyProSTT] Language set to auto-detect mode');
        } else {
            const config = RECOGNITION_LANGUAGES[key];
            if (!config) {
                console.warn(`[DeputyProSTT] Unknown language key: ${key}`);
                return;
            }
            ...
            this.languageAlternates = 'alternates' in config
                ...
                : [];
            console.log(`[DeputyProSTT] Language set: ${key} → ${this.languageBcp47}`,
        ...
        if (this.isActive && this.ws) {
            console.log('[DeputyProSTT] Language changed while active — reconnecting');
    ...
    /** No-op — Deputy API server handles VAD internally */
    ...
    public write(chunk: Buffer): void {
        ...
        if (!this.isConnected || !this.ws || this.ws.readyState !== WebSocket.OPEN) {
            ...
            if (this.buffer.length <= 3 || this.buffer.length % 100 === 0) {
                ...
                console.log(`[DeputyProSTT:${this.channel}] Buffering chunk (buffer=${this.buffer.length}, isConnected=${this.isConnected}, ws=${wsState})`);
            }
            return;
        }
        ...
        if (this._chunksSent <= 5 || this._chunksSent % 200 === 0) {
            console.log(`[DeputyProSTT:${this.channel}] Sent chunk #${this._chunksSent} (${chunk.length}B) to server`);
        }
        this.ws.send(chunk);
    }
    ...
    private connect(): void {
        ...
        console.log(`[DeputyProSTT] Connecting (attempt ${this.reconnectAttempts + 1})...`);
        ...
        this.ws.on('message', (data: WebSocket.Data) => {
            try {
                ...
                if (!msg.text || msg.is_final) {
                    console.log(`[DeputyProSTT:${this.channel}] Server msg:`, JSON.stringify(msg).slice(0, 120));
                }

                if (msg.error) {
                    console.error('[DeputyProSTT] Server error:', msg.error, msg.message || '');
                ...
                if (msg.status === 'connected') {
                    ...
                    this.reconnectAttempts = 0;
                    console.log(`[DeputyProSTT] Connected via ${msg.provider}`);
                    this.flushBuffer();
                    return;
                }
                ...
                if (msg.language_detected) {
                    ...
                    console.log(`[DeputyProSTT] Auto-detected language: ${detected}`);
                ...
                console.error('[DeputyProSTT] Parse error:', err);
            }
        });

        this.ws.on('error', (err: Error) => {
            console.error('[DeputyProSTT] WebSocket error:', err.message);
            this.isConnecting = false;
            this.isConnected  = false;
            this.emit('error', err);
        });
        ...
        this.ws.on('close', (code: number) => {
            this.isConnecting = false;
            this.isConnected  = false;
            console.log(`[DeputyProSTT] Connection closed (code ${code})`);
    ...
    private scheduleReconnect(): void {
        ...
        if (this.reconnectAttempts >= this.MAX_RECONNECT) {
            console.error('[DeputyProSTT] Max reconnect attempts reached — giving up');
            this.emit('error', new Error('DeputyProSTT: max reconnect attempts exceeded'));
            return;
        }

        const delay = this.RECONNECT_BASE_MS * Math.pow(2, this.reconnectAttempts);
        this.reconnectAttempts++;
        console.log(`[DeputyProSTT] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts}/${this.MAX_RECONNECT})...`);
