...
export function initializeIpcHandlers(appState: AppState): void {
  ...
  safeHandle("license:get-details", async () => {
    return { isPremium: true, plan: "max_ultra" };
  });
  ...
  safeHandle(
    ...
    async (event, { width, height }: { width: number; height: number }) => {
      ...
      if (settingsWin && !settingsWin.isDestroyed() && settingsWin.webContents.id === senderWebContents.id) {
        ...
        // DeputyInterface logic - Resize ONLY the overlay window using dedicated method
  ...
  // Generate suggestion from transcript - Deputy-style text-only reasoning
  ...
  safeHandle("get-log-file-path", async () => {
    try {
      return path.join(app.getPath('documents'), 'deputy_debug.log');
    } catch {
      return null;
    }
  });
  ...
  safeHandle("open-log-file", async () => {
    try {
      const logPath = path.join(app.getPath('documents'), 'deputy_debug.log');
  ...
  safeHandle("set-deputy-api-key", async (_, apiKey: string) => {
    try {
      const { CredentialsManager } = require('./services/CredentialsManager');
      const cm = CredentialsManager.getInstance();
      const prevSttProvider = cm.getSttProvider();
      cm.setDeputyApiKey(apiKey);

      // Update LLMHelper immediately (same pattern as other provider keys)
      const llmHelper = appState.processingHelper.getLLMHelper();
      llmHelper.setDeputyKey(apiKey || null);
      ...
      // If setDeputyApiKey auto-promoted the STT provider to 'deputy', reconfigure
      ...
      if (newSttProvider !== prevSttProvider) {
        console.log(`[IPC] set-deputy-api-key: STT provider changed ${prevSttProvider} → ${newSttProvider}, reconfiguring pipeline`);
        await appState.reconfigureSttProvider();
      }

      // Auto-activate Deputy Pro for pro/max/ultra API plans.
      ...
      if (apiKey) {
        try {
          ...
          if (result.success) {
            console.log('[IPC] set-deputy-api-key: Pro auto-activated via API plan.');
            ...
            console.log('[IPC] set-deputy-api-key: existing Gumroad/Dodo license preserved — Pro not overwritten.');
          } else {
            console.log('[IPC] set-deputy-api-key: Pro not activated —', result.error);
          }
        } catch (e: any) {
          // LicenseManager not available in this build — non-fatal
          console.warn('[IPC] set-deputy-api-key: LicenseManager unavailable for Pro auto-activation:', e?.message);
        }
      } else {
        // API key was cleared — deactivate any deputy_api Pro license so premium is revoked.
        try {
          const { LicenseManager } = require('../premium/electron/services/LicenseManager');
          const lm = LicenseManager.getInstance();
          // Only deactivate if the stored license is from a deputy_api subscription.
          // Never touch Gumroad/Dodo lifetime licenses here.
          const details = lm.getLicenseDetails();
          if (details.isPremium && details.provider === 'deputy_api') {
            await lm.deactivate();
            console.log('[IPC] set-deputy-api-key: key cleared — deputy_api Pro license deactivated.');
            BrowserWindow.getAllWindows().forEach(win => {
              if (!win.isDestroyed()) win.webContents.send('license-status-changed', { isPremium: false });
            });
          }
        } catch (e: any) {
          console.warn('[IPC] set-deputy-api-key: LicenseManager unavailable for Pro deactivation on key clear:', e?.message);
        }
      }

      return { success: true };
    } catch (error: any) {
      console.error("Error saving Deputy API key:", error);
      return { success: false, error: error.message };
    } finally {
  ...
  safeHandle("get-deputy-usage", async () => {
    try {
      const { CredentialsManager } = require('./services/CredentialsManager');
      const key = CredentialsManager.getInstance().getDeputyApiKey();
      ...
      const res = await fetch('https://api.deputy.software/v1/usage', {
        headers: { 'x-deputy-key': key },
  ...
  safeHandle("invalidate-deputy-usage-cache", () => {
    _usageCache.clear();
    return { ok: true };
  });
  ...
  safeHandle("trial:start", async () => {
    try {
      ...
      const res = await fetch('https://api.deputy.software/v1/trial/start', {
        method:  'POST',
      ...
      if (data.ok && data.trial_token && !data.expired) {
        ...
        // Auto-configure deputy as the model + STT provider during trial
        const prevSttProvider = cm.getSttProvider();
        cm.setDeputyApiKey('__trial__');   // sentinel — activates deputy model routing
        const newSttProvider = cm.getSttProvider();
        if (newSttProvider !== prevSttProvider) {
          await appState.reconfigureSttProvider();
        }
        const llmHelper = appState.processingHelper?.getLLMHelper?.();
        if (llmHelper) llmHelper.setDeputyKey('__trial__');
      }
  ...
  safeHandle("trial:status", async () => {
    try {
      ...
      const res = await fetch('https://api.deputy.software/v1/trial/status', {
  ...
  safeHandle("trial:convert", async (_, choice: string) => {
    try {
      ...
      await fetch('https://api.deputy.software/v1/trial/convert', {
        method:  'POST',
  ...
  // End trial via BYOK path: wipe Pro-ingested data, clear trial token + deputy key.
  safeHandle("trial:end-byok", async () => {
    try {
      ...
      const token = cm.getTrialToken();
      if (token) {
        fetch('https://api.deputy.software/v1/trial/convert', {
          method:  'POST',
          headers: { 'Content-Type': 'application/json', 'x-trial-token': token },
      ...
      cm.setDeputyApiKey('');
      const llmHelper = appState.processingHelper?.getLLMHelper?.();
      if (llmHelper) llmHelper.setDeputyKey(null);
      await appState.reconfigureSttProvider();
  ...
  // trial token or deputy key. Called automatically when trial expires so that
  ...
  safeHandle("get-stored-credentials", async () => {
    try {
      ...
      return {
        ...
        hasDeputyKey: hasKey(creds.deputyApiKey),
  ...
  safeHandle("set-stt-provider", async (_, provider: 'none' | 'google' | 'groq' | 'openai' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'deputy') => {
    try {
      const { CredentialsManager } = require('./services/CredentialsManager');
  ...
  safeHandle("set-model", async (_, modelId: string) => {
    try {
      ...
      // Broadcast to all windows so DeputyInterface can update its selector (session-only update)
      BrowserWindow.getAllWindows().forEach(win => {
        if (!win.isDestroyed()) {
  ...
  safeHandle("set-default-model", async (_, modelId: string) => {
    try {
      ...
      // Broadcast to all windows so DeputyInterface can update its selector
      BrowserWindow.getAllWindows().forEach(win => {
        if (!win.isDestroyed()) {
          win.webContents.send('model-changed', modelId);
        }
  ...
  safeHandle("profile:get-profile", async () => {
    try {
      ...
      if (typeof orchestrator.repairProfileFromSourceIfNeeded === 'function') {
        await orchestrator.repairProfileFromSourceIfNeeded();
      }
  ...
  safeHandle("profile:research-company", async (_, companyName: string) => {
    try {
      ...
      // Wire search provider: Tavily (user key) → Deputy API (fallback) → none (LLM-only)
      const { CredentialsManager } = require('./services/CredentialsManager');
      const cm = CredentialsManager.getInstance();
      ...
      if (tavilyApiKey) {
        ...
        const deputyKey = cm.getDeputyApiKey();
        if (deputyKey) {
          const { DeputySearchProvider } = require('../premium/electron/knowledge/DeputySearchProvider');
          ...
          const trialToken = deputyKey === '__trial__' ? cm.getTrialToken() : undefined;
          engine.setSearchProvider(new DeputySearchProvider(deputyKey, trialToken ?? undefined));
          console.log('[IPC] Company research: using Deputy API search (no Tavily key configured)');
        }
      }
