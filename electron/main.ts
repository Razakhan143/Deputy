...
import { DeputyProSTT } from "./audio/DeputyProSTT"
...
type STTProvider = (GoogleSTT | RestSTT | DeepgramStreamingSTT | SonioxStreamingSTT | ElevenLabsStreamingSTT | OpenAIStreamingSTT | DeputyProSTT) & {
  finalize?: () => void;
...
export class AppState {
  ...
  private createSTTProvider(speaker: 'interviewer' | 'user'): STTProvider | null {
    ...
    if (sttProvider === 'natively') {
      ...
      if (!nativelyKey) {
        ...
        stt = new DeputyProSTT(nativelyKey, speaker === 'interviewer' ? 'system' : 'mic');
      }
    ...
    // Auto language detection: DeputyProSTT emits 'languageDetected' when the
    ...
    if (stt instanceof DeputyProSTT) {
  ...
  private async reconfigureAudio(inputDeviceId?: string, outputDeviceId?: string): Promise<void> {
    ...
    try {
      ...
      console.warn(`[Main] Failed to initialize MicrophoneCapture with preferred ID. Falling back to default. Error: ${err instanceof Error ? err.message : String(err)}`);
      try {
  ...
  public setRecognitionLanguage(key: string): void {
    ...
    // 'auto' is only meaningful for DeputyProSTT — other providers fall back to en-US.
    const sttProvider = CredentialsManager.getInstance().getSttProvider();
