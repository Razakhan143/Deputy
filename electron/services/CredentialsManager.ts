...
export interface StoredCredentials {
    ...
    sttProvider?: 'none' | 'google' | 'groq' | 'openai' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'deputy';
    ...
    // Free trial state
    trialToken?:     string;   // server-issued signed token (deputy_trial_…)
...
export class CredentialsManager {
    ...
    public getSttProvider(): 'none' | 'google' | 'groq' | 'openai' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'deputy' {
    ...
    public getDeputyApiKey(): string | undefined {
        return this.credentials.deputyApiKey;
    }
    ...
    public setSttProvider(provider: 'none' | 'google' | 'groq' | 'openai' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'deputy'): void {
    ...
    public setDeputyApiKey(key: string): void {
        const trimmed = key.trim();
        this.credentials.deputyApiKey = trimmed || undefined;

        if (trimmed) {
            // Auto-promote deputy to default model unless user already chose a non-Gemini/Groq model
            ...
            const isAutoDefault = !current
                ...
                || current === 'llama';
            if (isAutoDefault) {
                this.credentials.defaultModel = 'deputy';
                console.log('[CredentialsManager] Auto-set default model to deputy');
            }

            // Auto-promote deputy STT if still on 'none' or the default Google STT
            if (!this.credentials.sttProvider || this.credentials.sttProvider === 'none' || this.credentials.sttProvider === 'google') {
                this.credentials.sttProvider = 'deputy';
                console.log('[CredentialsManager] Auto-set STT provider to deputy');
            }
        } else {
            // Key cleared — revert deputy-auto-set defaults back to safe fallbacks
            if (this.credentials.defaultModel === 'deputy') {
                this.credentials.defaultModel = 'gemini-3.1-flash-lite-preview';
                console.log('[CredentialsManager] Deputy key cleared — reset default model to Gemini Flash');
            }
            if (this.credentials.sttProvider === 'deputy') {
                this.credentials.sttProvider = 'none';
                console.log('[CredentialsManager] Deputy key cleared — reset STT provider to none');
            }
        }

        this.saveCredentials();
        console.log('[CredentialsManager] Deputy API Key updated');
    }
