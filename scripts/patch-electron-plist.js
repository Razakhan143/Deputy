...
if (!content.includes('NSScreenCaptureUsageDescription')) {
  content = content.replace(
    ...
    '<key>NSScreenCaptureUsageDescription</key>\n\t<string>Deputy needs Screen Recording permission to capture system audio for meeting transcription.</string>\n\t<key>NSMicrophoneUsageDescription</key>'
  );
  modified = true;
...
if (content.includes('This app needs access to the microphone')) {
  content = content.replace(
    ...
    '<string>Deputy needs microphone access to transcribe your voice during meetings.</string>'
  );
  modified = true;
