...
import DeputyInterface from "./components/DeputyInterface"
...
import { DeputyQuotaBanner } from "./components/DeputyQuotaBanner"
...
const App: React.FC = () => {
  ...
  // API check
  const [hasDeputyApi, setHasDeputyApi] = useState<boolean>(false);
  ...
  // Deputy API upsell campaign is intentionally disabled in core flow.
  useEffect(() => {
    if (activeAd === 'deputy_api') {
      dismissAd('deputy_api');
    }
  }, [activeAd, dismissAd]);
  ...
  useEffect(() => {
    ...
    // Also check for Deputy API key
    window.electronAPI?.getStoredCredentials?.()
      .then((creds) => setHasDeputyApi(!!creds?.hasDeputyKey))
      .catch(() => {});

    // ── Onboarding toasters ──────────────────────────────────
    if (isLauncherWindow || isDefault) {
      const permsShown = localStorage.getItem('deputy_perms_shown_v1');
      if (!permsShown) {
  ...
  const handleStartMeeting = async () => {
    try {
      localStorage.setItem('deputy_last_meeting_start', Date.now().toString());
  ...
  const handleEndMeeting = async () => {
    ...
    try {
      ...
      const startStr = localStorage.getItem('deputy_last_meeting_start');
      if (startStr) {
        const duration = Date.now() - parseInt(startStr, 10);
        const threshold = import.meta.env.DEV ? 10000 : 180000;
        if (duration >= threshold) {
          localStorage.setItem('deputy_show_profile_toaster', 'true');
        }
        localStorage.removeItem('deputy_last_meeting_start');
      }
  ...
  return (
    ...
    <div className="h-full min-h-0 w-full relative bg-[#000000]">
      ...
      <PermissionsToaster
        ...
        onDismiss={() => {
          localStorage.setItem('deputy_perms_shown_v1', '1');
