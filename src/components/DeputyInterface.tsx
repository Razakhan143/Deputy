...
interface DeputyInterfaceProps {
    onEndMeeting?: () => void;
    overlayOpacity?: number;
}

const DeputyInterface: React.FC<DeputyInterfaceProps> = ({ onEndMeeting, overlayOpacity = OVERLAY_OPACITY_DEFAULT }) => {
    ...
    const [hideChatHidesWidget, setHideChatHidesWidget] = useState(() => {
        const stored = localStorage.getItem('deputy_hideChatHidesWidget');
    ...
    // Persist Settings
    useEffect(() => {
        localStorage.setItem('deputy_undetectable', String(isUndetectable));
        localStorage.setItem('deputy_hideChatHidesWidget', String(hideChatHidesWidget));
    ...
    useLayoutEffect(() => {
        ...
        const observer = new ResizeObserver((entries) => {
            for (const entry of entries) {
                ...
                console.log('[DeputyInterface] ResizeObserver:', Math.ceil(rect.width), Math.ceil(rect.height));
    ...
    useEffect(() => {
        ...
        const unsubscribe = window.electronAPI.onSessionReset(() => {
            console.log('[DeputyInterface] Resetting session state...');
            setMessages([]);
            setInputValue('');
            setAttachedContext([]);
            setManualTranscript('');
    ...
    const handleAnswerNow = async () => {
        if (isManualRecording) {
            ...
            window.electronAPI.finalizeMicSTT().catch(err => console.error('[DeputyInterface] Failed to send finalizeMicSTT:', err));
...
export default DeputyInterface;
