// src/components/DeputyQuotaBanner.tsx
// Startup banner shown when any Deputy quota bucket reaches ≥90% usage.
...
export const DeputyQuotaBanner: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const check = async () => {
            ...
            try {
                const result = await window.electronAPI?.getDeputyUsage?.();
                console.log('[DeputyQuotaBanner] usage:', JSON.stringify(result));

                if (cancelled || !result?.ok || !result.quota) {
                    console.log('[DeputyQuotaBanner] no quota data — skipping');
                    return;
                }
                ...
                console.log('[DeputyQuotaBanner] near-limit:', near);
                ...
                console.log('[DeputyQuotaBanner] error:', e?.message);
            }
        };
                ...
                <div className="bg-[#1A1A1A] border border-amber-500/25 shadow-2xl rounded-2xl p-4 flex flex-col gap-3">
                    ...
                    <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[13px] font-semibold text-[#E0E0E0]">Deputy quota almost full</span>
                        </div>
                        <button
