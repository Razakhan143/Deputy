...
const SettingsPopup = () => {
    ...
    const [useGroqFastText, setUseGroqFastText] = useState(() => {
        return localStorage.getItem('deputy_groq_fast_text') === 'true';
    });
    ...
    useEffect(() => {
        if (window.electronAPI?.onUndetectableChanged) {
            const unsubscribe = window.electronAPI.onUndetectableChanged((newState: boolean) => {
                ...
                localStorage.setItem('deputy_undetectable', String(newState));
            });
            return () => unsubscribe();
        }
    }, []);
    ...
    useEffect(() => {
        ...
        if (window.electronAPI?.onGroqFastTextChanged) {
            const unsubscribe = window.electronAPI.onGroqFastTextChanged((enabled: boolean) => {
                ...
                localStorage.setItem('deputy_groq_fast_text', String(enabled));
            });
            return () => unsubscribe();
        }
    }, []);
    ...
    useEffect(() => {
        ...
        localStorage.setItem('deputy_groq_fast_text', String(useGroqFastText));
        try {
    ...
    return (
        <div className="w-fit h-fit bg-transparent flex flex-col">
            <div ref={contentRef} className={`w-[200px] max-h-[280px] backdrop-blur-md border rounded-[16px] overflow-hidden shadow-2xl p-2 flex flex-col animate-scale-in origin-top-left ${popupPanelClass}`}>
                ...
                {/* Groq (Fast Text) Toggle — enabled with Groq key OR Deputy API key */}
                <div className={`flex items-center justify-between px-3 py-2 rounded-lg transition-colors duration-200 group ${!(hasStoredKey.groq || hasStoredKey.deputy) ? 'opacity-50 grayscale cursor-not-allowed' : `${itemHoverClass} cursor-default`}`} title={!(hasStoredKey.groq || hasStoredKey.deputy) ? "Requires Groq or Deputy API key" : ""}>
                    <div className="flex items-center gap-3">
                        <Zap
                    ...
                    <button
                        onClick={() => {
                            if (!(hasStoredKey.groq || hasStoredKey.deputy)) return;
                        ...
                        disabled={!(hasStoredKey.groq || hasStoredKey.deputy)}
                    >
                ...
                <div
                    // @ts-ignore
                    onClick={() => window.electronAPI.openExternal('https://buymeacoffee.com/razakhanzas')}
