...
export const AIProvidersSettings: React.FC = () => {
    ...
    // Fast mode is available with a local Groq key OR via the Deputy API (server-side Groq pool)
    const canUseFastMode = !!(hasStoredKey.groq || hasStoredKey.deputy);
            ...
            try {
                ...
                if (creds) {
                    setHasStoredKey({
                        ...
                        deputy: creds.hasDeputyKey || false
                    });
    ...
    // Effect to enforce fast mode disabled if neither Groq key nor Deputy API is configured.
    ...
    useEffect(() => {
        ...
        if (!canUseFastMode && fastResponseMode) {
            ...
            localStorage.setItem('deputy_groq_fast_text', 'false');
            // @ts-ignore
        ...
        <div className="space-y-5 animated fadeIn pb-10">
            ...
            <div className="space-y-5">
                ...
                <div className="bg-bg-item-surface rounded-xl p-5 border border-border-subtle flex items-center justify-between">
                    ...
                    <ModelSelect
                        ...
                        options={(() => {
                            ...
                            if (hasStoredKey.deputy) {
                                opts.push({ id: 'deputy', name: 'Deputy API' });
                            }
                ...
                <div
                    ...
                    title={!canUseFastMode ? "Requires a Groq API Key or Deputy API to be configured" : ""}
                >
                    <div>
                        ...
                        {!canUseFastMode && (
                            <p className="text-[10px] text-orange-500 mt-0.5 font-medium">Requires a Groq API Key or Deputy API to be configured.</p>
                        )}
                    </div>
                    <div
                        onClick={async () => {
                            if (!canUseFastMode) {
                                alert("Please configure a Groq API Key or Deputy API first to enable Fast Response Mode.");
                                return;
                            }
                            const newState = !fastResponseMode;
                            setFastResponseMode(newState);
                            localStorage.setItem('deputy_groq_fast_text', String(newState));
                            // @ts-ignore
