...
interface SidebarProps {
    activeTab: 'general' | 'deputy-api' | 'ai-providers' | 'about';
    setActiveTab: (tab: 'general' | 'deputy-api' | 'ai-providers' | 'about') => void;
    onClose: () => void;
}
        ...
        <div className="w-64 bg-bg-sidebar flex flex-col border-r border-border-subtle h-full">
            <div className="p-6">
                ...
                <nav className="space-y-1">
                    ...
                    <button
                        onClick={() => setActiveTab('deputy-api')}
                        className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-3 ${activeTab === 'deputy-api' ? 'bg-bg-item-active text-text-primary' : 'text-text-secondary hover:text-text-primary hover:bg-bg-item-active/50'}`}
                    >
                        <Zap size={16} className="text-blue-500" /> Deputy API
                    </button>
                    <button
